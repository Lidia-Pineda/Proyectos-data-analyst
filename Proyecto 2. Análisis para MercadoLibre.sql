...
LEFT JOIN add_to_cart atc
    ON si.user_id = atc.user_id
LEFT JOIN begin_checkout bc
    ON atc.user_id = bc.user_id
LEFT JOIN add_shipping_info asi
    ON bc.user_id = asi.user_id
LEFT JOIN add_payment_info api
    ON asi.user_id = api.user_id
LEFT JOIN purchase p
    ON api.user_id = p.user_id;
...
SELECT
 ...
 country,
    ...
    usuarios_select_item * 100.0 / usuarios_first_visit AS conversion_select_item,

    usuarios_add_to_cart * 100.0 / usuarios_first_visit AS conversion_add_to_cart,

    usuarios_begin_checkout * 100.0 / usuarios_first_visit AS conversion_begin_checkout,

    usuarios_add_shipping_info * 100.0 / usuarios_first_visit AS conversion_add_shipping_info,

    usuarios_add_payment_info * 100.0 / usuarios_first_visit AS conversion_add_payment_info,

    usuarios_purchase * 100.0 / usuarios_first_visit AS conversion_purchase
